#!/usr/bin/env bash
#
# Runs the patched gen_node_support.pl (see
# patches/13_gen_node_support_libpg_query.patch) from an already-patched
# Postgres source tree to generate:
#
...
if ! grep -q libpg_query_outdir "$gen"; then
	echo "error: $gen does not contain the libpg_query modifications" >&2
	echo "       (is patches/13_gen_node_support_libpg_query.patch applied?)" >&2
	exit 1
fi
...
(cd "$pgsrc/src/backend/nodes" && perl "$gen" \
	--outdir "$tmp" \
	--libpg_query_outdir "$tmp" \
	--libpg_query_overrides "$here/scripts/node_support_overrides.pl" \
	"${headers[@]}")
