# libpg_query-specific overrides for the fingerprint code generation done by
# the patched gen_node_support.pl (see patches/13_gen_node_support_libpg_query.patch).
#
# This file is loaded by the generator via --libpg_query_overrides and keeps
# all per-node/per-field fingerprinting policy in the libpg_query repository,
# so routine changes here don't require regenerating the Postgres patch.
#
# Entry kinds:
#
