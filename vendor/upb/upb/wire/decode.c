...
static int _upb_Decoder_GetDelimitedOp(upb_Decoder* d, const upb_MiniTable* mt,
  ...
  /* libpg_query local patch: SQL string literals can contain arbitrary,
   * non-UTF-8 bytes in some server encodings (e.g. LATIN1). The previous
   * protobuf-c backend never validated proto3 `string` fields, so to preserve
   * round-tripping we decode string fields as bytes here, skipping upb's UTF-8
   * validation (the in-memory representation is identical, a upb_StringView).
   * The generated minitables contain no fast-decode entries, so this generic
   * path is the only one taken; re-apply on upb upgrade (see vendor/upb/VERSION). */
