const char* tests[] = {
  ...
  "SELECT (3 OPERATOR(pg_catalog.*) 3) + 2",
  "SELECT 3 + 3 * 2",
  "SELECT (3 + 3) * 2",
  "SELECT LIMIT ALL",
  ...
  "SELECT ts AT TIME ZONE (a + b), ts AT TIME ZONE (y OR z), ts AT TIME ZONE (y IS NULL), ts AT TIME ZONE (NOT y)",
  "SELECT ts AT TIME ZONE (y AT TIME ZONE 'UTC'), ts AT TIME ZONE (y AT LOCAL), ts AT TIME ZONE 'a' AT TIME ZONE 'b'",
  "SELECT ts AT TIME ZONE - y, ts AT TIME ZONE y COLLATE \"C\", ts AT TIME ZONE y::text, ts AT TIME ZONE y[1]",
  "SELECT (a OR b) AT TIME ZONE 'UTC', (a IS NULL) AT TIME ZONE 'UTC', - a AT TIME ZONE 'UTC', (a + b) AT LOCAL",
  "SELECT - (b AT TIME ZONE 'UTC'), (b AT TIME ZONE 'UTC')::text, x AT TIME ZONE 'UTC' + 1",
  "SELECT a - (b - c), a - b - c, (a = b) = c, - (a + b), - - a, a LIKE b = c, (a LIKE b) LIKE c",
  "SELECT a BETWEEN (b OR c) AND d, a BETWEEN (b COLLATE \"C\") AND d, a BETWEEN b + c AND d + e, a BETWEEN b AND (c OR d)",
  "SELECT (a IS DISTINCT FROM b) IS NOT DISTINCT FROM c, a IS NOT DISTINCT FROM (b OR c)",
  "SELECT (a || b) + ANY(c), a || b = ANY(c), (a || b) + ANY (SELECT 1), a || b IN (SELECT 1), (a OR b) IN (1, 2)",
  "CREATE TABLE my_table (a int DEFAULT (x COLLATE \"C\") + 1, b bool DEFAULT (NOT true), c bool DEFAULT (1 = ANY (SELECT 1)))",
  "SELECT (x COLLATE \"C\")::text, (a IN (SELECT 1))::text",
  "SELECT (NOT a) IS NULL, (a AND b) IS NULL, (a IS NULL) IS NULL, a = b IS NULL, (NOT a) IS TRUE, a < b IS TRUE",
  "SELECT (NOT a) IS DOCUMENT, a || b IS DOCUMENT, (NOT a) IS JSON, a || b IS JSON",
  "SELECT (a || b) COLLATE \"C\", (b AT TIME ZONE 'UTC') COLLATE \"C\", - a COLLATE \"C\", a::text COLLATE \"C\"",
  "SELECT a AND (b OR c), a OR (b AND c), a AND (b AND c), a AND NOT b, NOT (a AND b), NOT a = b, (NOT a) = b",
  "SELECT (- a)::int, (a IS NULL)::text, a::int::text, ((a, b) OVERLAPS (c, d))::text",
  "CREATE TABLE my_table (a text DEFAULT ((b, c) OVERLAPS (d, e))::text, f bool DEFAULT ((b, c) OVERLAPS (d, e)))",
  "CREATE TABLE my_table (a int DEFAULT 1 + 2 * 3, b int DEFAULT (1 + 2) * 3, c bool DEFAULT (a IS NULL), d bool DEFAULT a = b IS DISTINCT FROM c, e int DEFAULT 1 + (a IS NULL)::int)",
  "SELECT POSITION(a || b IN c), POSITION((a AND b) IN c), xmlexists ('//x' PASSING (a || b))",
  "SELECT * FROM xmltable(('/r') PASSING (a || b) COLUMNS c int PATH 'x' || y DEFAULT 1 + 2)",
  "CREATE INDEX i ON t USING btree ((ts AT TIME ZONE 'UTC'), (ts AT LOCAL), (x IS DOCUMENT), lower(x))",
  "SELECT (a COLLATE \"C\")[1], (a IS NULL)[1], (NOT a)[1], (a OR b)[1], (a = ANY (SELECT 1))[1], (EXISTS (SELECT 1))[1], (CASE WHEN a THEN b END)[1], ('{1,2}')[1]",
  "SELECT a[1][2], (a[1])[2], a.b[1].c, (a).b, $1[1], $1.f, (SELECT ARRAY[1])[1], (f(x)).y",
