...
PgQueryRawScanResult pg_query_scan_raw(const char* input)
{
  ...
  PG_TRY();
  {
    ...
    for (i = 0; ; i++)
    {
      ...
      result.tokens[i].start = yylloc;
      if (tok == SCONST || tok == USCONST || tok == BCONST || tok == XCONST || tok == IDENT || tok == UIDENT || tok == C_COMMENT) {
        result.tokens[i].end = yyextra.yyllocend;
      } else {
        result.tokens[i].end = yylloc + ((struct yyguts_t*) yyscanner)->yyleng_r;
      }
      result.tokens[i].token = tok;

      switch (tok) {
      #define PG_KEYWORD(a,b,c,d) case b: result.tokens[i].keyword_kind = c + 1; break;
      #include "parser/kwlist.h"
      #undef PG_KEYWORD
      default: result.tokens[i].keyword_kind = 0;
      }
    }
