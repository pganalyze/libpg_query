...
static bool const_record_walker(Node *node, pgssConstLocations *jstate)
{
	...
	/*
	 * Most node types reach raw_expression_tree_walker(), which checks the
	 * stack itself, but SelectStmt recurses into this function directly for
	 * every clause, so a long UNION chain never passes through that check.
	 */
	check_stack_depth();
	...
	switch (nodeTag(node))
	{
		...
		default:
			{
				/*
				 * Assign rather than return from inside PG_TRY: returning skips
				 * PG_END_TRY, which leaves PG_exception_stack pointing at this
				 * frame after it is gone, so a later error would longjmp into a
				 * dead stack frame.
				 */
				result = false;
				PG_TRY();
				...
				PG_CATCH();
				{
					...
					/*
					 * Hitting the stack limit means the rest of the tree was not
					 * walked. Swallowing that would return a query that still
					 * contains some of its constants as if it had been
					 * normalized, so let the caller see the error instead.
					 */
