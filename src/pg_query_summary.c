...
pg_query_summary_internal(const char *input, int parser_options, int truncate_limit)
{
	...
	{
		/*
		 * The walks can raise (e.g. "stack depth limit exceeded" on deeply
		 * nested input). stderr_buffer was malloc'ed by pg_query_raw_parse()
		 * and so far only this frame knows about it, so free it before the
