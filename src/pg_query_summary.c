		...
		case T_SelectStmt:
			{
				...
				/*
				 * For SETOP_UNION, SETOP_EXCEPT, and SETOP_INTERSECT,
				 * the raw_expression_tree_walker() call at the end of
				 * this function will handle stmt->larg/stmt->rarg.
				 */
		...
		case T_ColumnRef:
			{
				...
				/*
				 * Check that we are not in the A_Star case here.
				 * (Columns can be stars, but nothing else can.)
				 */
...
pg_query_summary_walk(Summary * summary, Node *tree)
{
	 ...
	 * - Because of this, we have to store range_vars and process it after
...
{
	/*
	 * We're converting from an enum we control (ContextType) to an enum
	 * which is auto-generated by protobuf (PgQuery__SummaryResult__Context).
	 *
